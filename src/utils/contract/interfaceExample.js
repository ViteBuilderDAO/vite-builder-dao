...
const { ViteAPI, wallet, accountBlock } = require('@vite/vitejs')
...
export default {
    ...
    methods: {
      ...
      async receiveTransaction(account) {
          // connect to node
      ...
      async sendTx(account, address, amount) {
      ...
      async callContract(account, methodName, abi, params, amount) {
      ...
      async fakeMainTest() {
          ...
          await callContract(myAccount, 'sayHello', CONTRACT.abi, [recipientAccount.address], '150000000000000000000').catch(err => console.error(err))
          // send 10 VITE 
          await sendTx(myAccount, recipientAccount.address, '10000000000000000000').catch(err => console.error(err))
          // recipient receives the tx
          await receiveTransaction(recipientAccount).catch(err => console.error(err))
      },
